...
class Board:
    ...
    def __init__(self, player_color):
        ...
        self.weights = {King: 500, Queen: 90, Rook: 50, Bishop: 30, Knight: 35, Pawn: 10}
        self.blackScore = 0
        self.whiteScore = 0
    ...
    def score_position(self):
        score_white_position = 0
        score_black_position = 0
        for x in range(8):
            for y in range(8):
                if self.tilemap[x][y].piece == None: continue
                elif self.tilemap[x][y].piece.color == WHITE:
                    if self.tilemap[x][y].piece == Pawn:
                        score_white_position += white_pawn[x][y]
                    elif self.tilemap[x][y].piece == Knight:
                        score_white_position += white_knight[x][y]
                    elif self.tilemap[x][y].piece == Bishop:
                        score_white_position += white_bishop[x][y]
                    elif self.tilemap[x][y].piece == Rook:
                        score_white_position += white_rook[x][y]
                    elif self.tilemap[x][y].piece == Queen:
                        score_white_position += white_queen[x][y]
                    elif self.tilemap[x][y].piece == King:
                        score_white_position += white_king[x][y]
                elif self.tilemap[x][y].piece.color == BLACK:
                    if self.tilemap[x][y].piece == Pawn:
                        score_black_position += black_pawn[x][y]
                    elif self.tilemap[x][y].piece == Knight:
                        score_black_position += black_knight[x][y]
                    elif self.tilemap[x][y].piece == Bishop:
                        score_black_position += black_bishop[x][y]
                    elif self.tilemap[x][y].piece == Rook:
                        score_black_position += black_rook[x][y]
                    elif self.tilemap[x][y].piece == Queen:
                        score_black_position += black_queen[x][y]
                    elif self.tilemap[x][y].piece == King:
                        score_black_position += black_king[x][y]
        return [score_black_position, score_white_position]
